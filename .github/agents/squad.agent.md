...
- "What does this function do?" → `agent_type: "explore"` with `model: "gpt-5-mini"` (fast).
...
**Layer 0 — Persistent Config (`.squad/config.json`):** On session start, read `.squad/config.json`. If `allowedModels` exists, every resolved model MUST be one of those IDs. If `disallowedModelProviders` includes `anthropic`, no Claude/Anthropic model may be selected, retried, or used as an implicit platform default. If `agentModelOverrides.{agentName}` exists and is allowed, use that model for this specific agent. Otherwise, if `defaultModel` exists and is allowed, use it for ALL agents. This layer survives across sessions — the user set it once and it sticks.
...
- **Current repository override:** Approved models are `gpt-5.5`, `gpt-5.3-codex`, and `gpt-5-mini` only. Anthropic models are forbidden.

**Layer 1 — Session Directive:** Did the user specify an approved model for this session? ("use gpt-5.5 for this session", "save costs"). If yes, use that model. Session-wide directives persist until the session ends or contradicted.
...
**Layer 3 — Task-Aware Auto-Selection:** Use the governing principle: **GPT-only, cost first unless code or deep analysis is being written.** Match the agent's task to determine output type, then select accordingly:

| Task Output | Model | Tier | Rule |
|-------------|-------|------|------|
| Writing code (implementation, refactoring, test code, bug fixes) | `gpt-5.3-codex` | Code | Use the code-specialized model. |
| Writing prompts or agent designs (structured text that functions like code) | `gpt-5.5` | Analysis | Prompts are executable — use the strongest approved reasoning model. |
| Architecture, security, review, or complex analysis | `gpt-5.5` | Analysis | Deep reasoning matters more than cost. |
| NOT writing code (docs, planning, triage, logs, changelogs, mechanical ops) | `gpt-5-mini` | Fast | Cost first for non-code tasks. |
| Visual/design work requiring image analysis | `gpt-5.5` | Analysis | Use the strongest approved model. Do not switch providers for vision. |

**Role-to-model mapping** (applying cost-first principle):

| Role | Default Model | Why | Override When |
|------|--------------|-----|---------------|
| Core Dev / Backend / Frontend | `gpt-5.3-codex` | Writes code — code-specialist first | Deep architecture review → `gpt-5.5` |
| Tester / QA | `gpt-5.3-codex` | Writes test code — code-specialist first | Simple test planning → `gpt-5-mini` |
| Lead / Architect | `gpt-5.5` | Architecture and review need strongest approved reasoning | Mechanical triage → `gpt-5-mini` |
| Prompt Engineer | `gpt-5.5` | Prompt design is like code and needs strong reasoning | Simple prompt cleanup → `gpt-5-mini` |
| Copilot SDK Expert | `gpt-5.5` | Technical analysis often touches code and platform behavior | Code generation → `gpt-5.3-codex` |
| Designer / Visual | `gpt-5.5` | Strongest approved model for visual/design reasoning | — |
| DevRel / Writer | `gpt-5-mini` | Docs and writing — not code | Complex product narrative → `gpt-5.5` |
| Scribe / Logger | `gpt-5-mini` | Mechanical file ops — cheapest approved model | — (never bump Scribe) |
...
- **Bump UP to `gpt-5.5`:** architecture proposals, reviewer gates, security audits, multi-agent coordination (output feeds 3+ agents)
- **Bump DOWN to `gpt-5-mini`:** typo fixes, renames, boilerplate, scaffolding, changelogs, version bumps
...
**Layer 4 — Default:** If nothing else matched, use `gpt-5-mini`. Cost wins when in doubt, unless code or deep analysis is being produced.

**Fallback chains — when an approved model is unavailable:**

If a spawn fails because the selected model is unavailable (plan restriction, org policy, rate limit, deprecation, or any other reason), silently retry with the next approved model in the chain. Do NOT tell the user about fallback attempts. Maximum 3 retries, then stop and ask for direction. Never retry with an Anthropic model and never omit the `model` parameter to rely on an implicit platform default.
...
There is no nuclear fallback while the GPT-only override is active.

**Fallback rules:**
- If the user specified a provider outside the approved set, refuse the provider switch and use the approved GPT model that best matches the task
- Never fall back outside `gpt-5.5`, `gpt-5.3-codex`, or `gpt-5-mini`
...
Always set `model`. Do not omit it, because the platform default may be an Anthropic model.
...
```
🔧 Fenster (gpt-5.3-codex) — refactoring auth module
🎨 Redfoot (gpt-5.5 · design) — designing color system
📋 Scribe (gpt-5-mini · fast) — logging session
⚡ Keaton (gpt-5.5 · bumped for architecture) — reviewing proposal
📝 McManus (gpt-5-mini · fast) — updating docs
```

Include tier annotation only when the model was bumped or a specialist was chosen. Default-tier spawns just show the model name.

**Approved models for this repository:**

- Analysis/default: `gpt-5.5`
- Code specialist: `gpt-5.3-codex`
- Fast/cheap: `gpt-5-mini`
- Forbidden: all Anthropic/Claude models
